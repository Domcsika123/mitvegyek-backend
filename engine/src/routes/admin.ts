...
import { listCatalogs, replaceCatalog, deleteCatalog, getProductsForSite } from "../services/productService";
...
router.get("/import-progress/latest", (_req, res) => {
  // Return the most recent running job (or the last finished one)
  let latest: ImportJob | null = null;
  for (const job of importJobs.values()) {
    if (!latest || job.status === "running") latest = job;
  }
  if (!latest) return res.status(404).json({ error: "No active import" });
  res.json(latest);
});
...
router.post("/import-products", async (req, res) => {
  ...
  try {
    ...
    return res.json({ ok: true, jobId, site_key, count, embedded: embeddedCount, batchSize: batchSize, typesExtracted });
  } catch (err) {
...
/** Típus kinyerés újrafuttatása meglévő katalóguson */
router.post("/catalogs/:site_key/extract-types", async (req, res) => {
  try {
    const { site_key } = req.params;
    const partner = findPartnerBySiteKey(site_key);
    ...
    if (!products || products.length === 0) {
      return res.status(404).json({ error: "Nincs importált katalógus ehhez a partnerhez." });
    }

    const typeOptions = await extractCatalogTypes(products);
    if (typeOptions.length === 0) {
      return res.json({ ok: true, typesExtracted: 0, message: "Nem sikerült típusokat kinyerni." });
    }

    updateTypeFieldOptions(site_key, typeOptions);
    return res.json({ ok: true, typesExtracted: typeOptions.length, types: typeOptions });
  } catch (err) {
    console.error("Hiba a /extract-types hívásban:", err);
...
router.post("/full-widget-config/:site_key/apply-preset", (req, res) => {
  try {
    ...
    const validPresets: PresetName[] = ["generic", "fashion", "electronics", "gift", "beauty", "home", "sport", "food", "kids", "books", "jewelry", "pet", "auto", "garden", "pharmacy"];
